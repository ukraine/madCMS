...
function GenerateListOfSomeThing($for, $limit="", $orderby = "`priority` DESC", $SourceOrTarget="source_id", $stlimit="0",$cat="", $getID="") {
	...
	for ($i=0; $i<$num_links; $i++){
		...
		"tests" =>
			"
				<form method='post' action='' class='tests'><p style='font-size:80%;'><B>" . intval($i+1) . "</B>
				(#<a name='$row[id]'>$row[id]) $row[ip]<!-- - $row[timespent] m. - $row[name] - $row[email] -  <a href='http://ip2location.com/$row[ip]' target='_blank'>$row[ip]</a>--> <span title='$row[comments]'>$row[email]</span>" . cmpWithGoogle(htmlspecialchars($row['testtranslation'])) . "</p> 
		...
		"fixedrates_new" =>
			...
			<tr>
				...
				<td class='size90 orderlink' valign='middle'>
					...
					"<button name='add-to-cart' class='add-to-cart' data-value='$row[id]'
					title='Order Russian/Ukrainian to English $documentName translation and certification'
					>"
...
function showRates($id, $rate, $comment) {
	...
	return "<input type='text' name='rater_comment' value='$comment' style='width: 404px;'>
				<select name='rate'>" . generateRateList($rate) . "</select>
				<input type='hidden' name='id' value='$id'>
				<input type='hidden' value='ratetest' name='dosometh'>
				<input type='submit' value='rate' style='width: auto;'>";
...
function cmpWithGoogle($test) {
	...
	$var =  " G: " . STRLEN . " chars, this: $actualTestLength chars<br>";
